#!/usr/bin/env bash
# PortableAgents + DeepSeek — Codex (OpenAI CLI) backed by DeepSeek's OpenAI-compatible API.
# Any arguments pass straight through to `codex`, e.g.:  ./codex-deepseek.sh "explain this code"
set -euo pipefail
...
# --- DeepSeek env (provides OPENAI_BASE_URL + OPENAI_API_KEY) ---------------
DEEPSEEK_ENV="${DEEPSEEK_ENV:-$ROOT/config/deepseek_env.sh}"
if [ -f "$DEEPSEEK_ENV" ]; then
  # shellcheck disable=SC1090
  source "$DEEPSEEK_ENV"
else
  echo "codex-deepseek: missing $DEEPSEEK_ENV (set DEEPSEEK_ENV=... or create config/deepseek_env.sh)" >&2
  exit 1
fi

# Default model for DeepSeek via OpenAI-compatible endpoint
